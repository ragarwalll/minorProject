...
<head>
    ...
    <link rel="stylesheet" href="../css-global/preloader-icon.css">
    <script type='text/javascript' src='../js-global/jquery.min.js'></script>
    <script src='../js-global/preloader-icon.js'></script>
    <link rel="stylesheet" href="../css-global/header.css">
    <script src='../js-global/header-toggle.js'></script>
        ...
        <div class="left-sided">
            <div class="index">
                <ul>
                    ...
                    <ul>
                        <li><a href="javascript:void(0)" data="5-1">Keyboard and Mouse</a></li>
                        <ul>
...
Since then, personal computers have become tremendously more efficient. They are much smaller, and yet have seen extreme performance gains. In addition to these improvements, computers have become affordable enough for many families worldwide.</p>
            ...
            <h4 class="5-1-2">Mouse</h4>
            <p>The mouse is an input device which is primarily used by physically moving the device across a surface. This causes a pointer symbol, called a "cursor", to move across the screen. The other input comes from pressing a button while the cursor is over an object on the monitor, or "clicking". All mice have at least one button, with the most common layout having three. Gaming mice may have 7 buttons</p>
            <br>
            <ul>
                <li> <p>One button mice: The Apple mighty mouse is the only mouse known to most people which uses a single button. This button is usually activated by pushing on the front of the mouse, or pushing the entire mouse down. The "right click" is done by pressing a "control" or "Ctrl" key on the keyboard while pressing the main button. These are only used with Macintosh OS, such as Mac OS X.</p> </li>
                <li><p>Two button mice: The second most common layout, more common in older computers, which has a button on the left and right, usually for the index and middle finger. While less useful than a three button mouse, they are, when teamed with a standard keyboard, capable of performing almost all computer tasks.</p></li>
                <li><p>Three button mice: The most common layout, Fundamentally the same as a two button mice, but with a third button, the middle button, added between the "left" and "right" click buttons. While the mouse technically has three buttons, this may be confusing to some users, as the "middle"/"center" click button is also a scroll wheel. This design allows the user to scroll through documents, make selections, and do other tasks by moving a finger. The other way to is to press an arrow key or "Page up/Page down" key on a keyboard. The center button can also be press inwards to create a "middle click" button. The middle click is used in some programs to perform a simple function, such as the Firefox web browser, which uses the middle click to open a link in a new tab</p></li>
                <li><p>Four (or more) button mice: Usually seen only on gaming and multimedia specialization mice. The extra buttons generally do not serve any native purpose, and rather are assigned a function to perform by the user. For example, a user who uses the mouse to play games might assign the additional buttons to switch between weapons in a shooter, or cast spells in a fantasy game.</p></li>
            </ul>    
            <p>There are two other major differences in mice, which is Optical/Laser mice, and Ball mice. This is how the mouse tells where it is, with the laser measuring the distance it crosses when it is moved, and the ball measuring how it rotates. The laser is generally more accurate and less of a hassle to use, and can be used on more surfaces, but the ball mouse is cheaper. Ball mice are rarely seen today.</p>
            <br>
            <p>The last important consideration when buying a mouse is size. You should always try to put your hand on a mouse and move it around, to see how well it feels in your hand. If it feels awkward, small, big, long, or short, look for something better! Not only will your hands thank you, but you will be more efficient.</p>
            <br>
            <p>Mice can be wired or wireless - see the above section on keyboards.</p>
            <br>
            <p>A trackball is very similar to a mouse but has a ball instead of the middle button. This ball allows you to move the mouse up, down or side to side. There are usually 2 or more buttons as well. Many users find they cause less pain than a traditional mouse.</p>
            <br>
            <p>A Touchpad is often found on a laptop instead of a mouse. This pad is found under the keyboard in most laptops i.e. closer to the body than the actual keys. Moving a finger across it moves the pointer. Multiple fingers can be used to send other commands e.g. two finger prolonged press opens up a page in a new tab.</p>

            <h3 class="5-2">Media Devices (Floppy, CD-ROM, DVD, USB)</h3>
            <p>These devices carry data, in the same way that a hard drive does, but are much more portable. They are the primary method of storing data outside of a computer, and the main method of transferring information between computers without the use of a network, such as the Internet.</p>
            <br>
            <p>There are three main types of these in use today:</p>
            <ul>
                <li><p>CD-ROM: mostly read only memory unless labeled "rewritable" - Capable of storing 700MB of data, CDs have been the most common method of storing data for most of the last decade or so. They are being largely replaced by DVDs and USB drives (see below).</p></li>
                <li><p>DVD: Capable of storing 4.7GB of data in their single layer form and 8.5GB in their double layer form, they are the most common method today for most store-bought programs, as well as videos.</p></li>
                <li><p>USB/Flash: While not usually used by commercial software, USB 'sticks' and Flash 'cards' have become popular ways of storing data because of their ease of use and low cost. While sizes range from 2GB on old units to 256GB on larger, more expensive modern units, the average stick today is 4 or 8GB, with an average 4GB USB stick costing about $15 US.</p></li>
            </ul>
            <p>The floppy disk has been phased out.</p>

            <h3 class="5-3">Monitor</h3>

            <p>The monitor is the main method for the computer to produce output, in the same way a book has pages. A book filled with letters, but in a way you can't possibly understand or even see is of no use to you, and the same is true for a computer. While older monitors, CRTs, were rather bulky like TVs, newer monitors, or LCDs, are much more compact, and can be easily lifted.</p>
            <br>
            <p>For much of the history of computers, the most common monitors were CRTs, short for Cathode Ray Tubes. They work on the same principle as a television. They were generally heavy, had a lower image quality, and were in general less reliable than an LCD. They come in two forms, the normal version, which has a curved monitor, and "Flat Screen", where the display is completely level (although it still has the bulky back end). It is suggested that if you plan to remove a CRT, it is better for the environment to safely dispose of it (not just into a bin, an electronic waste bin), as CRT's contain high levels of toxic chemicals. The most common type of monitor today is an LCD, or Liquid Crystal Display, which is much lighter, although slightly more expensive. They have a smaller form, a higher image quality, and are overall better than CRT monitors.</p>
            <br>
            <p>Tip: To take proper care of your monitor, always be sure that the screen is not left on a static image for long periods of time. This can "burn" the image into the monitor, meaning that it will have a ghosting effect, even when that image is not displayed. This can not only be highly annoying, but in some cases, make it so the monitor needs to be replaced. To avoid this, either set a moving screensaver, which will trigger after a set amount of time, or simply turn the monitor off</p>

            <h3 class="5-4">Printer</h3>

            <p>If you have a printer attached to your computer you can print your information and keep a physical copy of data. Depending on what type of printer you have, you can print in color, double-sided or book form. The output quality of some printers goes from draft (to save ink) all the way to photo quality.</p>
            <br>
            <p>Printers come in all price ranges and in many types. The most common for home use are ink jets (bubble jets) and laser. There are specialized printers for data plotting (mapping), photos only, labels and more.</p>
            <br>
            <p>A lot of printers are "all-in-ones" which simply means they combine more than one function. They can be any combination of printer, copier, fax and scanner. If you are looking to buy an all-in-one make sure it has all the features you are looking for. You don't want to get it home and find out you still need a separate fax machine or scanner that you thought was included!</p>
            <br>            
        </div>
